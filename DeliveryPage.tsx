...
import { fetchSellers, fetchProducts, fetchCountries, fetchAdCampaigns, fetchPaymentProviders, fetchOrders, fetchComplianceReports, updateSellerStatus, updateAdCampaignStatus, setProductApproval, logAuditAction } from '@/lib/db';
...
export function AdminPage() {
  ...
  const pendingProducts = products.filter((p) => !p.approval_status || p.approval_status === 'pending');
                  ...
                  {pendingProducts.slice(0, 20).map((p) => (
                    <div key={p.id} className="card p-4 flex items-center gap-4 bg-white">
                      ...
                      <div className="flex gap-2">
                        <button onClick={async () => { const ok = await setProductApproval(p.id, 'approved'); if (!ok) { showToast(locale === 'fr' ? 'Échec de l\u2019approbation' : 'Approval failed', 'error'); return; } await logAuditAction({ actorId: user?.id, actorName: user?.fullName, action: 'product.approve', targetType: 'product', targetId: p.id, targetName: p.name }); setProducts(products.map(x => x.id === p.id ? { ...x, approval_status: 'approved', is_active: true } : x)); showToast(locale === 'fr' ? 'Produit approuvé' : 'Product approved'); }} className="px-3 py-2 rounded-lg bg-green-100 text-green-700 text-xs font-semibold flex items-center gap-1 hover:bg-green-200"><CheckCircle className="w-4 h-4" /> {locale === 'fr' ? 'Approuver' : 'Approve'}</button>
                        <button onClick={async () => { const ok = await setProductApproval(p.id, 'rejected'); if (!ok) { showToast(locale === 'fr' ? 'Échec du rejet' : 'Rejection failed', 'error'); return; } await logAuditAction({ actorId: user?.id, actorName: user?.fullName, action: 'product.reject', targetType: 'product', targetId: p.id, targetName: p.name }); setProducts(products.map(x => x.id === p.id ? { ...x, approval_status: 'rejected', is_active: false } : x)); showToast(locale === 'fr' ? 'Produit rejeté' : 'Product rejected'); }} className="px-3 py-2 rounded-lg bg-red-100 text-red-700 text-xs font-semibold flex items-center gap-1 hover:bg-red-200"><XCircle className="w-4 h-4" /> {locale === 'fr' ? 'Rejeter' : 'Reject'}</button>
                      </div>
                    </div>
                  ))}
