...
import { fetchProducts, fetchSellerOrders, updateOrderStatus, fetchAdCampaigns, uploadProductImage, createProduct, createPayoutRequest } from '@/lib/db';
...
export function SellerCenterPage() {
  ...
  const nextStatus: Record<string, 'preparing' | 'inTransit' | 'delivered' | null> = {
    confirmed: 'preparing', preparing: 'inTransit', inTransit: 'delivered', delivered: null, cancelled: null, pending: 'preparing',
  };

  const advanceOrder = async (orderId: string, current: string) => {
    const next = nextStatus[current];
    if (!next) return;
    try {
      await updateOrderStatus(orderId, next);
      setOrders((prev) => prev.map((o) => o.id === orderId ? { ...o, status: next } : o));
      showToast(locale === 'fr' ? 'Statut de la commande mis à jour' : 'Order status updated');
    } catch (e) {
      console.error(e);
      showToast(locale === 'fr' ? "Échec de la mise à jour" : 'Update failed', 'error');
    }
  };
                  ...
                  <div className="card overflow-hidden bg-white">
                    {orders.map((o, i) => (
                      <div key={o.id} className={`flex items-center gap-3 p-4 ${i > 0 ? 'border-t border-[#e2e8f0]' : ''}`}>
                        ...
                        <span className="px-2 py-1 text-[10px] font-bold uppercase rounded-full" style={{ background: `${statusColors[o.status]}15`, color: statusColors[o.status] }}>{t.delivery[o.status as 'pending' | 'confirmed' | 'preparing' | 'inTransit' | 'delivered' | 'cancelled']}</span>
                        <span className="font-bold text-[#0f172a] w-16 text-right">${o.total.toFixed(0)}</span>
                        {nextStatus[o.status] && (
                          <button onClick={() => advanceOrder(o.id, o.status)} className="text-xs font-semibold text-[#0e9f6e] hover:underline whitespace-nowrap">
                            {locale === 'fr' ? `→ ${t.delivery[nextStatus[o.status] as 'preparing' | 'inTransit' | 'delivered']}` : `→ ${t.delivery[nextStatus[o.status] as 'preparing' | 'inTransit' | 'delivered']}`}
                          </button>
                        )}
                      </div>
                    ))}
