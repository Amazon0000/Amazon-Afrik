...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const cartLineKey = (productId: string, variation?: string) => variation ? `${productId}-${variation}` : productId;

  const addToCart = (productId: string, qty = 1, variation?: string) => {
    setCart((prev) => {
      const key = cartLineKey(productId, variation);
      const existing = prev.find((c) => cartLineKey(c.productId, c.variation) === key);
      if (existing) return prev.map((c) => cartLineKey(c.productId, c.variation) === key ? { ...c, qty: c.qty + qty } : c);
      return [...prev, { productId, qty, variation }];
    });
  };
  const removeFromCart = (productId: string, variation?: string) => {
    const key = cartLineKey(productId, variation);
    setCart((prev) => prev.filter((c) => cartLineKey(c.productId, c.variation) !== key));
  };
  const updateCartQty = (productId: string, qty: number, variation?: string) => {
    const key = cartLineKey(productId, variation);
    setCart((prev) => prev.map((c) => cartLineKey(c.productId, c.variation) === key ? { ...c, qty: Math.max(1, qty) } : c));
  };
