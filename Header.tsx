      ...
      <div className="bg-[#00255e] text-white">
        <div className="max-w-[1500px] mx-auto px-4 h-10 flex items-center justify-between text-xs sm:text-sm font-medium overflow-hidden">
          ...
          <div className="hidden lg:flex items-center gap-1.5 text-[#12b77e] font-bold px-3 shrink-0">
            <span>🇿🇦 🇰🇪 🇳🇬 🇨🇮 🇸🇳 🇬🇭</span>
            <span className="text-[#94a3b8] text-xs font-normal">SaaS Marketplace</span>
          </div>
        </div>
      </div>
          ...
          <div className="fixed top-0 right-0 bottom-0 w-full max-w-[420px] bg-white text-[#0f172a] z-50 shadow-2xl flex flex-col h-full animate-fade-up">
            ...
            <div className="p-4 border-b border-[#dddddd] flex items-center justify-between bg-[#f3f3f3]">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-base font-semibold text-[#0f172a]">{locale === 'fr' ? 'Votre Panier Zando' : 'Your Zando Cart'}</h2>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              {cartItemsResolved.length === 0 ? (
                ...
                cartItemsResolved.map((item) => (
                  <div key={item.variation ? `${item.productId}-${item.variation}` : item.productId} className="flex gap-3 border-b border-[#eee] pb-4 last:border-none">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center justify-between mt-2">
                        <div className="flex items-center border border-gray-300 rounded overflow-hidden scale-90 origin-left">
                          <button onClick={() => updateCartQty(item.productId, item.qty - 1, item.variation)} className="px-2 py-0.5 bg-gray-100 hover:bg-gray-200 text-xs font-bold">-</button>
                          <span className="px-3.5 text-xs font-bold text-gray-800">{item.qty}</span>
                          <button onClick={() => updateCartQty(item.productId, item.qty + 1, item.variation)} className="px-2 py-0.5 bg-gray-100 hover:bg-gray-200 text-xs font-bold">+</button>
                        </div>
                        <button onClick={() => removeFromCart(item.productId, item.variation)} className="text-xs text-red-500 hover:text-red-700 flex items-center gap-1 font-medium transition-colors">
