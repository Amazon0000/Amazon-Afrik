...
export type Product = {
  ...
  is_sponsored: boolean; is_active: boolean; approval_status?: 'pending' | 'approved' | 'rejected'; created_at: string;
...
// Orders belonging to a given seller (used by the Seller Center dashboard).
// This is intentionally distinct from fetchOrders(), which filters by *customer* user_id.
...
// Guest-safe order lookup by tracking id, via a security-definer RPC (see migration 012)
// so anonymous visitors can track a delivery without exposing every order through RLS.
export async function fetchOrderByTrackingId(trackingId: string): Promise<Order | null> {
  const { data, error } = await supabase.rpc('get_order_by_tracking', { p_tracking_id: trackingId });
  if (error) {
    console.error('fetchOrderByTrackingId failed', error);
    return null;
  }
  if (!data) return null;
...
export type CartLine = { productId: string; qty: number; variation?: string };

// Creates one order per seller present in the cart (a marketplace order can only belong
// to a single seller — see `orders.seller_id`), each with its own tracking id.
export async function createOrdersFromCart(opts: {
...
}): Promise<{ orderId: string; trackingId: string; sellerId: string | null; total: number }[]> {
  const { items, userId, guestName, guestEmail, guestPhone, deliveryAddress, paymentMethod } = opts;

  const groups = new Map<string, { line: CartLine; product: Product }[]>();
  ...
  const results: { orderId: string; trackingId: string; sellerId: string | null; total: number }[] = [];
  let seq = 0;
  for (const [sellerId, groupItems] of groups) {
    seq += 1;
    const total = groupItems.reduce((sum, i) => sum + i.product.price * i.line.qty, 0);
    const trackingId = `ORD-${Date.now().toString().slice(-6)}${groups.size > 1 ? `-${seq}` : ''}`;

    const { data: order, error } = await supabase.from('orders').insert({
      user_id: userId,
      guest_name: !userId ? guestName ?? null : null,
      guest_email: !userId ? guestEmail ?? null : null,
      guest_phone: !userId ? guestPhone ?? null : null,
    ...
    for (const item of groupItems) {
      const { error: itemError } = await supabase.from('order_items').insert({
        ...
        product_name: item.product.name + (item.line.variation ? ` (${item.line.variation})` : ''),
        qty: item.line.qty,
    ...
    results.push({ orderId: order.id, trackingId, sellerId: sellerId === 'unknown' ? null : sellerId, total });
  }
...
export async function createProduct(opts: {
  ...
  const { data: product, error } = await supabase.from('products').insert({
    ...
    stock: opts.stock,
    sku: opts.sku ?? null,
    // New listings stay hidden from shoppers until an admin approves them —
    // previously is_active was hardcoded true here, so "approve/reject" in the
    // admin panel only changed a label and never actually hid rejected products.
    is_active: false,
...
// Approving/rejecting must also flip is_active — approval_status alone is just a label
// and has no effect on whether the product is actually visible in the storefront.
export async function setProductApproval(productId: string, status: 'approved' | 'rejected'): Promise<boolean> {
  const { error } = await supabase.from('products').update({
    approval_status: status,
    is_active: status === 'approved',
  }).eq('id', productId);
  if (error) { console.error('setProductApproval:', error.message); return false; }
  return true;
}
