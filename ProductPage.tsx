...
export function ProductPage() {
  ...
  const variantSelectionComplete = variantTypes.every((vt) => Boolean(selectedVariants[vt]));
  const selectedVariantAdjustment = variantTypes.reduce((sum, vt) => {
    const chosen = variants.find((v) => v.variant_type === vt && v.variant_value === selectedVariants[vt]);
    return sum + (chosen?.price_adjustment || 0);
  }, 0);
  const displayPrice = product.price + selectedVariantAdjustment;

  const handleAddToCart = () => {
    if (!variantSelectionComplete) return;
      ...
      <div className="max-w-[1500px] mx-auto px-4 sm:px-6 py-4">
            ...
            <div className="border border-gray-300 rounded p-4 bg-white space-y-4">
              ...
              <div className="space-y-2 pt-2">
                <button
                  ...
                  disabled={product.stock === 0 || !variantSelectionComplete}
                ...
                {variantTypes.length > 0 && !variantSelectionComplete && (
                  <p className="text-[11px] text-[#cc0c39] font-medium text-center">
                    {locale === 'fr' ? 'Veuillez sélectionner une option ci-dessus.' : 'Please select an option above.'}
                  </p>
                )}
              </div>
        ...
        {related.length > 0 && (
          <div className="mt-12 border-t border-gray-200 pt-8">
            <h2 className="text-lg font-bold text-gray-900 mb-4">{t.product.relatedProducts}</h2>
