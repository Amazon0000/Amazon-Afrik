      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
        <div className="premium-card mb-6 rounded-2xl p-4 sm:p-5">
          <div className="flex items-center gap-3">
            ...
            <div><h1 className="font-display text-2xl font-semibold text-[#0f172a]">{t.admin.title}</h1><p className="text-xs text-[#64748b]">{isSuperAdmin ? (locale === 'fr' ? 'Super Admin — accès total' : 'Super Admin — full access') : (locale === 'fr' ? 'Admin — accès limité' : 'Admin — limited access')}</p></div>
          </div>
        </div>
              ...
              <nav className="space-y-0.5 max-h-[70vh] overflow-y-auto no-scrollbar">
                {visibleNav.map((item) => (
                  <button key={item.id} onClick={() => handleNav(item.id)}
                    ...
                    {item.id === 'ads' && pendingAds.length > 0 && <span className="ml-auto text-xs bg-[#0e9f6e] text-white px-1.5 rounded-full font-bold">{pendingAds.length}</span>}
                  </button>
                ))}
              </nav>
            </div>
          ...
          <div className="flex-1 min-w-0">
              ...
              <div className="animate-fade-up space-y-6">
                ...
                <div className="grid lg:grid-cols-3 gap-4">
                  <div className="premium-card p-5 bg-white/90 rounded-2xl">
                    <h3 className="font-display text-sm font-bold text-[#0f172a] mb-3 flex items-center gap-2"><Clock className="w-4 h-4 text-[#0e9f6e]" /> {locale === 'fr' ? "File d'approbation" : 'Approval queue'}</h3>
                    <div className="space-y-2">
                      <div className="flex items-center justify-between p-2 rounded-lg bg-[#0e9f6e]/5"><span className="text-xs text-[#0f172a]">{locale === 'fr' ? 'Vendeurs en attente' : 'Pending sellers'}</span><Badge color="#0e9f6e">{pendingSellers.length}</Badge></div>
                      <div className="flex items-center justify-between p-2 rounded-lg bg-[#0e9f6e]/5"><span className="text-xs text-[#0f172a]">{locale === 'fr' ? 'Produits en attente' : 'Pending products'}</span><Badge color="#0e9f6e">{pendingProducts.length}</Badge></div>
                      <div className="flex items-center justify-between p-2 rounded-lg bg-[#0e9f6e]/5"><span className="text-xs text-[#0f172a]">{locale === 'fr' ? 'Campagnes en attente' : 'Pending campaigns'}</span><Badge color="#0e9f6e">{pendingAds.length}</Badge></div>
                    ...
                    <div className="space-y-2">
                      {orders.slice(0, 5).map((o) => (
                        <div key={o.id} className="flex items-center gap-2 py-1.5 border-b border-[#e2e8f0] last:border-0">
                          ...
                          <Badge color={o.status === 'delivered' ? '#22c55e' : o.status === 'pending' ? '#0e9f6e' : '#64748b'}>{o.status}</Badge>
                        </div>
                      ))}
            ...
            {tab === 'orders' && (
              <div className="animate-fade-up">
                <h2 className="font-display text-xl font-semibold text-[#0f172a] mb-4">{locale === 'fr' ? 'Gestion des commandes' : 'Orders Management'}</h2>
                      ...
                      <tbody>
                        {orders.slice(0, 30).map((o) => (
                          <tr key={o.id} className="border-t border-[#e2e8f0] hover:bg-[#f7f8fa]">
                            ...
                            <td className="px-4 py-3"><Badge color={o.status === 'delivered' ? '#22c55e' : o.status === 'pending' ? '#0e9f6e' : o.status === 'cancelled' ? '#ef4444' : '#64748b'}>{o.status}</Badge></td>
            ...
            {tab === 'sellers' && (
              <div className="animate-fade-up">
                <div className="flex items-center justify-between mb-4">
                  <h2 className="font-display text-xl font-semibold text-[#0f172a]">{t.admin.sellers}</h2>
                ...
                <div className="card overflow-hidden bg-white">
                  {filteredSellers.map((s, i) => (
                    <div key={s.id} className={`flex items-center gap-3 p-4 ${i > 0 ? 'border-t border-[#e2e8f0]' : ''}`}>
                      ...
                      <Badge color={s.plan === 'enterprise' ? '#0e9f6e' : s.plan === 'premium' ? '#0e9f6e' : '#64748b'}>{s.plan}</Badge>
                      <Badge color={s.status === 'approved' ? '#22c55e' : s.status === 'pending' ? '#0e9f6e' : s.status === 'rejected' ? '#ef4444' : '#64748b'}>{s.status}</Badge>
                      {s.status === 'pending' && (
            ...
            {tab === 'products' && (
              <div className="animate-fade-up">
                <h2 className="font-display text-xl font-semibold text-[#0f172a] mb-4">{t.admin.products}</h2>
                <div className="card overflow-hidden bg-white">
            ...
            {tab === 'product-approval' && isSuperAdmin && (
              <div className="animate-fade-up">
                <h2 className="font-display text-xl font-semibold text-[#0f172a] mb-4">{locale === 'fr' ? 'Approbation des produits' : 'Product Approval'}</h2>
                <div className="card p-4 mb-4 bg-[#0e9f6e]/5 flex items-center gap-3">
                  <PackageCheck className="w-5 h-5 text-[#0e9f6e]" />
            ...
            {tab === 'super-admins' && isSuperAdmin && (
              <div className="animate-fade-up">
                <h2 className="font-display text-xl font-semibold text-[#0f172a] mb-4">{locale === 'fr' ? 'Gestion des Super Admins' : 'Super Admin Management'}</h2>
            ...
            {tab === 'kyc' && (
              <div className="animate-fade-up">
                <h2 className="font-display text-xl font-semibold text-[#0f172a] mb-4">{t.admin.kyc}</h2>
                <div className="space-y-3">
                  {sellers.slice(0, 5).map((s) => (
            ...
            {tab === 'ads' && (
              <div className="animate-fade-up">
                <h2 className="font-display text-xl font-semibold text-[#0f172a] mb-4">{t.admin.ads}</h2>
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
                ...
                <div className="card p-5 bg-white">
                  ...
                  {pendingAds.length === 0 ? (
                    ...
                    <div className="space-y-2">
                      {pendingAds.map((ad) => (
                        <div key={ad.id} className="flex items-center gap-3 p-3 rounded-lg bg-[#0e9f6e]/5">
                          <Megaphone className="w-4 h-4 text-[#0e9f6e]" />
            ...
            {tab === 'disputes' && (
              <div className="animate-fade-up">
                <h2 className="font-display text-xl font-semibold text-[#0f172a] mb-4">{t.admin.disputes}</h2>
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
                ...
                <div className="space-y-3">
                  {complianceReports.slice(0, 20).map((r) => (
                    <div key={r.id} className="card p-4 flex items-start gap-3 bg-white">
                      ...
                      <Badge color={r.status === 'open' ? '#ef4444' : r.status === 'investigating' ? '#0e9f6e' : '#22c55e'}>{r.status}</Badge>
                      {r.status === 'open' && (
            ...
            {tab === 'payouts' && isSuperAdmin && (
              <div className="animate-fade-up">
                <h2 className="font-display text-xl font-semibold text-[#0f172a] mb-4">{locale === 'fr' ? 'Paiements vendeurs' : 'Seller Payouts'}</h2>
            ...
            {tab === 'geography' && isSuperAdmin && (
              <div className="animate-fade-up">
                <h2 className="font-display text-xl font-semibold text-[#0f172a] mb-4">{t.admin.geography}</h2>
            ...
            {tab === 'staff' && isSuperAdmin && (
              <div className="animate-fade-up">
                <div className="flex items-center justify-between mb-4">
                  <h2 className="font-display text-xl font-semibold text-[#0f172a]">{t.admin.staff}</h2>
            ...
            {tab === 'analytics' && (
              <div className="animate-fade-up space-y-6">
                <h2 className="font-display text-xl font-semibold text-[#0f172a]">{t.admin.analytics}</h2>
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                      ...
                      {['starter', 'premium', 'enterprise'].map((plan) => {
                        ...
                        return (
                          <div key={plan}>
                            ...
                            <div className="h-2 rounded-full bg-[#f7f8fa] overflow-hidden"><div className="h-full rounded-full" style={{ width: `${pct}%`, backgroundColor: plan === 'enterprise' ? '#0e9f6e' : plan === 'premium' ? '#0e9f6e' : '#64748b' }} /></div>
                          </div>
                        );
                      })}
                    </div>
                      ...
                      {['pending', 'confirmed', 'delivered', 'cancelled'].map((st) => {
                        ...
                        return (
                          <div key={st}>
                            ...
                            <div className="h-2 rounded-full bg-[#f7f8fa] overflow-hidden"><div className="h-full rounded-full" style={{ width: `${pct}%`, backgroundColor: st === 'delivered' ? '#22c55e' : st === 'pending' ? '#0e9f6e' : st === 'cancelled' ? '#ef4444' : '#64748b' }} /></div>
                          </div>
                        );
                      })}
            ...
            {tab === 'plans' && isSuperAdmin && (
              <div className="animate-fade-up">
                <h2 className="font-display text-xl font-semibold text-[#0f172a] mb-4">{t.admin.plans}</h2>
            ...
            {tab === 'documents' && (
              <div className="animate-fade-up">
                <h2 className="font-display text-xl font-semibold text-[#0f172a] mb-4">{t.admin.documents}</h2>
                <div className="card p-5 bg-white">
            ...
            {tab === 'settings' && isSuperAdmin && (
              <div className="animate-fade-up">
                <h2 className="font-display text-xl font-semibold text-[#0f172a] mb-4">{t.admin.settings}</h2>
                <div className="space-y-4">
                  ...
                  <div className="card p-5 bg-white">
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      {[{ plan: 'Starter', limit: 1, color: '#64748b' }, { plan: 'Premium', limit: 5, color: '#0e9f6e' }, { plan: 'Enterprise', limit: 20, color: '#0e9f6e' }].map((p) => (
