...
type AppState = {
  ...
  updateCartQty: (productId: string, qty: number, variation?: string) => void;
  clearCart: () => void;
  cartCount: number;
  wishlist: string[];
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const cartLineKey = (c: { productId: string; variation?: string }) => c.variation ? `${c.productId}-${c.variation}` : c.productId;

  const addToCart = (productId: string, qty = 1, variation?: string) => {
    setCart((prev) => {
      const key = cartLineKey({ productId, variation });
      const existing = prev.find((c) => cartLineKey(c) === key);
      if (existing) return prev.map((c) => cartLineKey(c) === key ? { ...c, qty: c.qty + qty } : c);
      return [...prev, { productId, qty, variation }];
    });
  };
  // variation must be passed for products that have variants, otherwise a customer with
  // two different variations of the same product in their cart would remove/update both at once.
  const removeFromCart = (productId: string, variation?: string) => {
    const key = cartLineKey({ productId, variation });
    setCart((prev) => prev.filter((c) => cartLineKey(c) !== key));
  };
  const updateCartQty = (productId: string, qty: number, variation?: string) => {
    const key = cartLineKey({ productId, variation });
    setCart((prev) => prev.map((c) => cartLineKey(c) === key ? { ...c, qty: Math.max(1, qty) } : c));
  };
