  ...
  return (
    <div className="motif-bg min-h-screen">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
        <h1 className="font-display text-3xl font-semibold text-[#0f172a] mb-6">{t.cart.title} {items.length > 0 && <span className="text-lg text-[#64748b] font-normal">({items.length} {t.cart.items})</span>}</h1>
        {items.length === 0 ? (
          ...
          <div className="grid lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 space-y-3">
              {items.map((item) => (
                <div key={item.variation ? `${item.productId}-${item.variation}` : item.productId} className="card p-4 flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-2">
                      <div className="flex items-center border border-[#0f172a]/15 rounded-lg overflow-hidden">
                        <button onClick={() => updateCartQty(item.productId, item.qty - 1, item.variation)} className="p-1.5 hover:bg-[#0f172a]/5"><Minus className="w-3.5 h-3.5 text-[#0f172a]" /></button>
                        <span className="px-3 text-sm font-semibold text-[#0f172a]">{item.qty}</span>
                        <button onClick={() => updateCartQty(item.productId, item.qty + 1, item.variation)} className="p-1.5 hover:bg-[#0f172a]/5"><Plus className="w-3.5 h-3.5 text-[#0f172a]" /></button>
                      </div>
                      <button onClick={() => removeFromCart(item.productId, item.variation)} className="flex items-center gap-1 text-xs text-red-400 hover:text-red-600"><Trash2 className="w-3.5 h-3.5" /> {t.cart.remove}</button>
                    </div>
                  </div>
            ...
            <div>
              <div className="card p-5 sticky top-20">
                <h2 className="font-display text-lg font-semibold text-[#0f172a] mb-4">{t.cart.orderSummary}</h2>
