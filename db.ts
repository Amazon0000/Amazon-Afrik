...
export async function uploadSellerAsset(file: File, sellerId: string, type: 'logo' | 'banner' | 'selfie' | 'id-front' | 'id-back'): Promise<string | null> {
  const ext = file.name.split('.').pop() || 'jpg';
...
export async function addSellerDocument(opts: {
  sellerId: string;
  docType: string;
  fileUrl: string;
  ...
  if (error) { console.error('addSellerDocument:', error.message); return false; }
  return true;
}
