  ...
  return (
    <div className="motif-bg min-h-screen">
      <div className="relative h-48 sm:h-64 overflow-hidden bg-[#0f172a]">
        <img src={seller.store_banner_url || ''} alt={seller.business_name} className="w-full h-full object-cover opacity-75" />
        <div className="absolute inset-0 bg-gradient-to-t from-[#0f172a] via-[#0f172a]/45 to-transparent" />
        <div className="absolute inset-x-0 bottom-0 h-20 bg-gradient-to-t from-[#f7f8fa] to-transparent" />
      </div>

      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="relative -mt-16 flex flex-col sm:flex-row items-center sm:items-end gap-4 pb-6">
          <div className="w-24 h-24 sm:w-28 sm:h-28 rounded-[28px] border-4 border-white overflow-hidden bg-[#f7f8fa] shadow-[0_22px_40px_-18px_rgba(15,23,42,0.4)] shrink-0">
            <img src={seller.store_logo_url || ''} alt={seller.business_name} className="w-full h-full object-cover" />
          </div>
          <div className="flex-1 text-center sm:text-left">
            <div className="flex items-center justify-center sm:justify-start gap-2">
              <h1 className="font-display text-2xl sm:text-3xl font-semibold text-[#0f172a]">{seller.business_name}</h1>
            ...
            <div className="flex items-center justify-center sm:justify-start gap-4 mt-2">
              <div className="flex items-center gap-1"><Star className="w-4 h-4 fill-[#0e9f6e] text-[#0e9f6e]" /><span className="text-sm font-medium text-[#0f172a]">{seller.rating}</span><span className="text-xs text-[#64748b]/60">({seller.total_reviews})</span></div>
        ...
        <div className="grid grid-cols-3 gap-3 mb-8">
          <div className="premium-card p-4 text-center rounded-2xl"><Package className="w-5 h-5 text-[#0e9f6e] mx-auto mb-1" /><p className="text-xl font-bold text-[#0f172a]">{seller.total_products}</p><p className="text-xs text-[#64748b]">{t.seller.products}</p></div>
          <div className="premium-card p-4 text-center rounded-2xl"><Star className="w-5 h-5 text-[#0e9f6e] mx-auto mb-1" /><p className="text-xl font-bold text-[#0f172a]">{seller.rating}</p><p className="text-xs text-[#64748b]">{t.product.reviews}</p></div>
          <div className="premium-card p-4 text-center rounded-2xl"><Calendar className="w-5 h-5 text-[#0e9f6e] mx-auto mb-1" /><p className="text-xl font-bold text-[#0f172a]">{seller.joined_year}</p><p className="text-xs text-[#64748b]">{locale === 'fr' ? 'Membre depuis' : 'Member since'}</p></div>
        </div>

        <h2 className="font-display text-2xl font-semibold text-[#0f172a] mb-5">{products.length} {t.seller.products}</h2>
