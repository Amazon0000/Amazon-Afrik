  ...
  const submit = async () => {
    ...
    try {
      ...
      const { data: sellerData, error: sellerError } = await supabase.from('sellers').insert({
        ...
        city: form.businessAddress || null,
  ...
  return (
    <div className="motif-bg min-h-screen">
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
        ...
        {/* 14-day trial banner */}
        <div className="card p-4 mb-6 flex items-center gap-3 bg-gradient-to-r from-[#0e9f6e]/10 to-[#ff9900]/10 border-[#0e9f6e]/20">
          <Sparkles className="w-5 h-5 text-[#0e9f6e] shrink-0" />
          <div>
          ...
          {step === 6 && (
            <div>
              ...
              <div className="p-4 rounded-xl bg-green-50 border border-green-200 mb-5 flex items-start gap-3">
                ...
                <div>
                  <p className="text-xs font-bold text-green-800 uppercase tracking-wide">
                    {locale === 'fr' ? 'Zando prend 0% de commission sur vos ventes' : 'Zando takes 0% commission on your sales'}
                  </p>
                  <p className="text-xs text-green-700 mt-1 leading-relaxed">
                    {locale === 'fr'
                      ? "Chaque franc généré par vos ventes vous revient en intégralité. Vos clients vous paient en direct via le moyen de paiement que vous connectez. Le seul coût sur Zando est votre abonnement de plateforme !"
                      : "Every dollar generated by your sales is 100% yours. Your clients pay you directly through the payment method you connect. The only cost on Zando is your monthly platform subscription!"}
                  </p>
                </div>
              </div>
              ...
              <div className="mt-5 space-y-4">
                <div className="p-4 rounded-xl bg-[#ff9900]/5 border border-[#ff9900]/20">
  ...
  return (
    <div>
      ...
      <div className="relative">
        {Icon && <Icon className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-[#64748b]" />}
