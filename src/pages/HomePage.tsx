...
import { ShieldCheck, CreditCard, Truck, Globe, ArrowRight, Sparkles, TrendingUp, Store, MapPin, Clock, Zap, Tag, Gift, Award, Megaphone, ChevronDown, Star } from 'lucide-react';
    ...
    <div className="bg-[#f7f8fa]">
      ...
      <section className="relative bg-white border-b border-[#e2e8f0]">
        <div className="section-shell">
          <div className="hero-panel relative h-[260px] sm:h-[380px] lg:h-[440px] rounded-[28px] overflow-hidden my-4">
            {heroSlides.map((p, i) => (
              <div key={p.id} className={`absolute inset-0 transition-opacity duration-700 ${i === heroIdx ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                ...
                <div className="relative h-full flex flex-col justify-center max-w-xl px-6 sm:px-12">
                  <span className="premium-pill inline-flex items-center gap-2 px-3 py-1.5 rounded-full mb-3 w-fit">
        ...
        <div className="section-shell py-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              ...
              <div key={i} className="premium-card flex items-center gap-3 rounded-2xl px-4 py-3">
                <div className="w-11 h-11 rounded-xl bg-[#0e9f6e]/10 flex items-center justify-center shrink-0 soft-glow">
                  <item.icon className="w-5 h-5 text-[#0e9f6e]" />
                </div>
                <p className="text-xs sm:text-sm font-semibold text-[#0f172a]">{item.label}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Sponsored carousel — Amazon-style promotional banners */}
      {sponsored.length > 0 && (
        <section className="section-shell py-6">
          <div className="flex items-center gap-2 mb-3">
            <Megaphone className="w-4 h-4 text-[#ff9900]" />
            <span className="text-xs font-semibold text-[#64748b] uppercase tracking-wide">{locale === 'fr' ? 'Sponsorisé' : 'Sponsored'}</span>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            {sponsored.slice(0, 4).map((p) => {
              ...
              return (
                <div key={p.id} onClick={() => navigate('product', { id: p.id })} className="card overflow-hidden cursor-pointer group bg-white">
                  <div className="relative h-32 overflow-hidden bg-[#f7f8fa]">
                    <img src={p.product_images?.[0]?.image_url || ''} alt="" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                    {discount > 0 && <span className="absolute top-2 left-2 px-2 py-0.5 text-[10px] font-bold rounded bg-[#ff9900] text-white">-{discount}%</span>}
                  </div>
                  <div className="p-3">
                    <p className="text-xs font-semibold text-[#0f172a] truncate">{p.name}</p>
                    <p className="text-xs text-[#64748b] mt-0.5">{p.sellers?.business_name}</p>
                    <p className="text-sm font-bold text-[#0e9f6e] mt-1">${p.price}</p>
                  </div>
                </div>
              );
            })}
      ...
      <div className="section-shell space-y-8 sm:space-y-10 py-8">
        {/* Flash Deals */}
        {deals.length > 0 && (
        ...
        {/* Categories — Amazon-style compact strip */}
        {allCategories.length > 0 && (
          <section>
            <div className="flex items-center justify-between mb-2">
              <h2 className="font-display text-sm font-bold text-[#0f172a]">{t.home.categoriesTitle}</h2>
              <button onClick={() => navigate('catalog')} className="text-xs font-semibold text-[#0e9f6e] hover:underline flex items-center gap-1">{t.home.ctaBrowse} <ArrowRight className="w-3 h-3" /></button>
            </div>
            <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
              {rootCategories.map((cat) => {
                const Icon = (Icons as unknown as Record<string, React.ElementType>)[cat.icon || ''] || Icons.Tag;
                return (
                  <button key={cat.id} onClick={() => navigate('catalog', { category: cat.id })} className="premium-pill shrink-0 flex items-center gap-2 px-3 py-2.5 rounded-xl hover:border-[#0e9f6e]/40 hover:bg-white/70 transition-all group">
                    <Icon className="w-4 h-4 text-[#0e9f6e]" />
                    <span className="text-xs font-semibold text-[#0f172a] group-hover:text-[#0e9f6e]">{cat.name}</span>
                  </button>
                );
              })}
            </div>
        ...
        {/* Products by Country — discrete dropdown */}
        <section>
          <div className="flex items-center justify-between mb-2">
            <h2 className="font-display text-sm font-bold text-[#0f172a]">{t.home.byCountry}</h2>
            <div className="relative">
              <select onChange={(e) => { if (e.target.value) navigate('catalog', { country: e.target.value }); }} className="appearance-none pl-7 pr-7 py-1.5 text-xs rounded-lg border border-[#0f172a]/10 bg-[#f7f8fa] text-[#64748b] focus:outline-none focus:border-[#0e9f6e] cursor-pointer" defaultValue="">
                <option value="" disabled>{locale === 'fr' ? 'Choisir un pays' : 'Select country'}</option>
                {countries.map((c) => <option key={c.id} value={c.id}>{c.flag} {c.name}</option>)}
              </select>
              <span className="absolute left-2 top-1/2 -translate-y-1/2 text-xs pointer-events-none">🌐</span>
        ...
        {sellers.length > 0 && (
          <section>
            ...
            <div className="flex gap-4 overflow-x-auto no-scrollbar pb-2 snap-x">
              {sellers.map((s) => (
                <div key={s.id} onClick={() => navigate('seller', { id: s.id })} className="card overflow-hidden cursor-pointer group bg-white min-w-[260px] max-w-[260px] snap-start shrink-0">
