            ...
            {campaigns.map((c) => (
              <div key={c.id} className="card p-5">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className={'px-2.5 py-1 text-[10px] font-bold uppercase rounded-full ' + (c.status === 'active' ? 'bg-green-100 text-green-700' : c.status === 'pending' ? 'bg-[#ff9900]/15 text-[#ff9900]' : 'bg-gray-100 text-gray-500')}>
                    {c.status}
                  </span>
                </div>
