    ...
    <div className="motif-bg min-h-screen">
      ...
      <section className="motif-dark py-20 px-4 text-center relative overflow-hidden">
        <div className="absolute inset-0 opacity-5">
          <div className="absolute top-10 left-10 w-72 h-72 rounded-full bg-[#ff7a00] blur-3xl" />
          <div className="absolute bottom-10 right-10 w-72 h-72 rounded-full bg-[#ff7a00] blur-3xl" />
        </div>
        <div className="max-w-3xl mx-auto relative">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#ff7a00]/15 text-[#ff7a00] text-xs font-semibold mb-4">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {benefits.map((b, i) => (
            <div key={i} className="card p-6 hover:shadow-lg transition-shadow">
              <div className="w-12 h-12 rounded-xl bg-[#ff7a00]/10 flex items-center justify-center mb-4">
                <b.icon className="w-6 h-6 text-[#ff7a00]" />
              </div>
      ...
      {/* How it works */}
      <section className="bg-white/50 border-y border-[#ff7a00]/20 py-16">
          ...
          <div className="space-y-4">
            {steps.map((s) => (
              <div key={s.num} className="flex items-start gap-4 card p-5 hover:shadow-md transition-shadow">
                <div className="w-12 h-12 rounded-xl bg-[#ff7a00]/10 flex items-center justify-center shrink-0">
                  <s.icon className="w-5 h-5 text-[#ff7a00]" />
                </div>
        ...
        <div className="grid sm:grid-cols-3 gap-6">
          {testimonials.map((tm, i) => (
            <div key={i} className="card p-6">
              <Quote className="w-8 h-8 text-[#ff7a00]/30 mb-3" />
              <div className="flex gap-0.5 mb-3">
                {Array.from({ length: tm.rating }).map((_, j) => <Star key={j} className="w-4 h-4 fill-[#ff7a00] text-[#ff7a00]" />)}
              </div>
              <p className="text-sm text-[#0f172a] mb-4 leading-relaxed">"{tm.text}"</p>
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-[#ff7a00]/15 flex items-center justify-center text-sm font-bold text-[#ff7a00]">{tm.name.charAt(0)}</div>
                <div>
      ...
      <section className="max-w-5xl mx-auto px-4 sm:px-6 py-16">
        ...
        <div className="grid sm:grid-cols-3 gap-6">
          {[
            ...
            <div key={p.name} className={'card p-6 relative ' + (p.highlight ? 'ring-2 ring-[#ff7a00] shadow-xl' : '')}>
              {p.highlight && <span className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-0.5 text-[10px] font-bold rounded-full bg-[#ff7a00] text-white">{locale === 'fr' ? 'POPULAIRE' : 'POPULAR'}</span>}
              <h3 className="font-display text-xl font-bold text-[#0f172a]">{p.name}</h3>
              <p className="text-3xl font-bold text-[#ff7a00] mt-2">{p.price}<span className="text-sm text-[#64748b]">/mo</span></p>
              <p className="text-xs text-[#ff7a00] font-semibold mt-1">{locale === 'fr' ? '14 jours gratuits' : '14 days free'}</p>
              <ul className="mt-4 space-y-2">
                {p.features.map((f, i) => <li key={i} className="flex items-center gap-2 text-sm text-[#0f172a]"><CheckCircle className="w-4 h-4 text-[#ff7a00] shrink-0" /> {f}</li>)}
              </ul>
      ...
      <section className="motif-dark py-16 text-center px-4 relative overflow-hidden">
        <div className="absolute inset-0 opacity-5">
          <div className="absolute top-0 left-1/4 w-64 h-64 rounded-full bg-[#ff7a00] blur-3xl" />
        </div>
        <div className="max-w-2xl mx-auto relative">
          <Award className="w-12 h-12 text-[#ff7a00] mx-auto mb-4" />
