    ...
    <div className="motif-bg min-h-screen">
      ...
      <section className="motif-dark py-20 px-4 text-center relative overflow-hidden">
        <div className="absolute inset-0 opacity-5">
          ...
          <div className="absolute bottom-10 right-10 w-72 h-72 rounded-full bg-[#ff9900] blur-3xl" />
        </div>
          ...
          {testimonials.map((tm, i) => (
            <div key={i} className="card p-6">
              ...
              <div className="flex gap-0.5 mb-3">
                {Array.from({ length: tm.rating }).map((_, j) => <Star key={j} className="w-4 h-4 fill-[#ff9900] text-[#ff9900]" />)}
              </div>
        ...
        <div className="grid sm:grid-cols-3 gap-6">
          {[
            ...
            <div key={p.name} className={'card p-6 relative ' + (p.highlight ? 'ring-2 ring-[#0e9f6e] shadow-xl' : '')}>
              ...
              <p className="text-xs text-[#ff9900] font-semibold mt-1">{locale === 'fr' ? '14 jours gratuits' : '14 days free'}</p>
