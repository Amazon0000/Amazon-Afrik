    ...
    <header className="sticky top-0 z-50 bg-[#0a2240] text-white font-sans">
      ...
      <div className="max-w-[1500px] mx-auto px-4 py-1.5 flex items-center justify-between gap-3 text-sm">
        ...
        <div className="flex-1 max-w-4xl relative" ref={searchRef}>
          <form onSubmit={handleSearch} className="flex w-full">
            <div className="flex items-center w-full rounded-md bg-white overflow-hidden focus-within:ring-2 focus-within:ring-[#e77600] transition-shadow">
              ...
              <input type="text" value={search} onChange={(e) => { setSearch(e.target.value); setShowSuggestions(true); }} onFocus={() => setShowSuggestions(true)}
                placeholder={t.common.searchPlaceholder} className="flex-1 px-3 py-2 text-[14px] bg-transparent focus:outline-none text-[#111111] h-10 font-sans" />
              <button type="submit" className="px-5 bg-[#febd69] hover:bg-[#f3a847] transition-colors h-10 flex items-center justify-center shrink-0">
                <Search className="w-5 h-5 text-[#111111]" />
              </button>
            </div>
          </form>
          {showSuggestions && suggestions.length > 0 && (
            <div className="absolute top-full mt-1 left-0 right-0 bg-white border border-[#dddddd] rounded shadow-lg animate-fade-up z-50 max-h-80 overflow-y-auto text-[#111111]">
              ...
              {suggestions.map((s) => (
                <button key={s} onClick={() => { setSearch(s); navigate('catalog', { q: s }); setShowSuggestions(false); }}
                  className="flex items-center gap-2.5 w-full px-3 py-2.5 text-sm text-[#111111] hover:bg-[#f3f3f3] transition-colors text-left font-medium">
        ...
        <div className="flex items-center gap-2 shrink-0">
            ...
            <div className="relative">
              ...
              {userMenuOpen && (
                <>
                  ...
                  <div className="absolute right-0 mt-2 w-60 bg-white border border-[#dddddd] rounded shadow-xl z-50 p-2 text-[#111111] animate-fade-up">
                    <div className="px-3 py-2.5 border-b border-[#eee] mb-1">
                      <p className="text-xs text-[#565959]">{locale === 'fr' ? 'Votre compte :' : 'Your account:'}</p>
                      <p className="text-sm font-bold text-[#111111] truncate">{user.fullName}</p>
          ...
          <button onClick={() => setCartDrawerOpen(true)} className="relative flex items-end gap-1 px-2.5 py-1.5 rounded-sm hover:outline hover:outline-1 hover:outline-white transition-all text-white">
            <div className="relative flex items-center">
              ...
              <span className="absolute -top-1 left-1/2 -translate-x-1/2 w-5 h-5 text-xs font-bold flex items-center justify-center rounded-full bg-[#f08804] text-black">
                {cartCount}
              </span>
            </div>
      ...
      <div className="bg-[#003087] text-white">
        <div className="max-w-[1500px] mx-auto px-4 h-10 flex items-center justify-between text-xs sm:text-sm font-medium overflow-hidden">
          <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar py-1">
            ...
            <button onClick={() => go('sell')} className="px-3 py-1.5 rounded-sm hover:outline hover:outline-1 hover:outline-white font-bold text-[#ff9900] whitespace-nowrap transition-all shrink-0">
              {t.nav.becomeSeller}
            </button>
          </div>
        </div>
      ...
      {megaOpen && (
        <>
          ...
          <div className="absolute top-full left-0 right-0 bg-white border-t border-[#dddddd] shadow-2xl z-50 animate-fade-up max-h-[75vh] overflow-y-auto text-[#111111]">
            <div className="max-w-7xl mx-auto px-6 py-8">
              <div className="flex items-center justify-between mb-4 pb-2 border-b border-[#eee]">
                <h3 className="text-base font-bold text-[#111111] flex items-center gap-2"><Store className="w-5 h-5 text-[#ff9900]" /> {t.home.categoriesTitle}</h3>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-x-6 gap-y-2">
                {MEGA_CATEGORIES.map((item) => (
                  <button key={item.label} onClick={() => handleMegaNav(item)}
                    className="flex items-center justify-between px-2 py-1.5 rounded text-sm text-[#111111] hover:bg-[#f3f3f3] hover:text-[#e47911] transition-colors text-left group">
                    <span className="truncate">{item.label}</span>
                    <ChevronRight className="w-3.5 h-3.5 text-[#bbb] group-hover:text-[#e47911] transition-colors shrink-0" />
                  </button>
                ))}
              </div>
            </div>
      ...
      {cartDrawerOpen && (
        <>
          ...
          {/* Slide panel */}
          <div className="fixed top-0 right-0 bottom-0 w-full max-w-[420px] bg-white text-[#111111] z-50 shadow-2xl flex flex-col h-full animate-fade-up">
            {/* Drawer Header */}
            <div className="p-4 border-b border-[#dddddd] flex items-center justify-between bg-[#f3f3f3]">
              <div className="flex items-center gap-2">
                <ShoppingCart className="w-5 h-5 text-[#e47911]" />
                <h2 className="text-base font-bold text-[#111111]">{locale === 'fr' ? 'Votre Panier Zando' : 'Your Zando Cart'}</h2>
                <span className="text-xs bg-[#e47911] text-white px-2 py-0.5 rounded-full font-bold">{cartCount}</span>
              </div>
                ...
                cartItemsResolved.map((item) => (
                  <div key={item.productId} className="flex gap-3 border-b border-[#eee] pb-4 last:border-none">
                    ...
                    <div className="flex-1 min-w-0">
                      <h4 className="text-xs font-bold text-[#111111] line-clamp-2 leading-tight hover:text-[#e47911] cursor-pointer" onClick={() => { setCartDrawerOpen(false); navigate('product', { id: item.productId }); }}>
                        {item.product!.name}
                      </h4>
                    ...
                    <div className="text-right shrink-0">
                      <p className="text-xs font-bold text-[#111111]">${(item.product!.price * item.qty).toFixed(0)}</p>
            ...
            {cartItemsResolved.length > 0 && (
              <div className="p-4 border-t border-[#dddddd] bg-[#fcfcfc] space-y-3 shrink-0">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-lg font-black text-[#111111]">${cartSubtotal.toFixed(2)}</span>
                </div>
...
function MenuItem({ icon: Icon, label, onClick }: { icon: React.ElementType; label: string; onClick: () => void }) {
  return (
    <button onClick={onClick} className="flex items-center gap-3 w-full px-3 py-2 text-sm text-[#111111] rounded hover:bg-[#f3f3f3] transition-colors text-left font-medium">
