...
import { Menu, X, Search, ShoppingBag, Globe, ChevronDown, User as UserIcon, Store, Shield, LayoutDashboard, LogOut, Heart, Package, MapPin, Bell, MessageSquare, ChevronRight, Headphones } from 'lucide-react';
          ...
          <div className="flex items-center gap-0.5">
            ...
            <div className="relative hidden sm:block">
              <select value={geo.countryId} onChange={(e) => setGeo({ countryId: e.target.value })}
                className="appearance-none pl-7 pr-2 py-2 text-xs rounded-lg border border-[#e2e8f0] bg-white text-[#0f172a] focus:outline-none focus:border-[#0e9f6e] cursor-pointer max-w-[70px]" title={currentCountry?.name}>
                {countries.map((c) => <option key={c.id} value={c.id}>{c.flag} {c.name}</option>)}
              </select>
              <span className="absolute left-2 top-1/2 -translate-y-1/2 text-sm pointer-events-none">{currentCountry?.flag}</span>
            </div>
            {/* Notifications */}
